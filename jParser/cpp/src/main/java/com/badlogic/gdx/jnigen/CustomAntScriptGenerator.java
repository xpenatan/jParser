 ...
 * @author mzechner
 */
public class CustomAntScriptGenerator {
    /**
...
//		}
        ...
        for(int i = 0; i < libsDirs.size(); i++) {
            pack.append("\t\t\t<fileset dir=\"" + libsDirs.get(i) + "\" includes=\"" + sharedLibFiles.get(i) + "\" erroronmissingdir=\"false\"/>\n");
        }

        if(config.sharedLibs != null) {
            for(String sharedLib : config.sharedLibs) {
                pack.append("\t\t\t<fileset dir=\"" + sharedLib + "\" erroronmissingdir=\"false\"/>\n");
            }
        }
