...
tasks.register<JavaExec>("runtime_helper_build_project_linux64_teavm_c") {
    group = "jParser"
    description = "Generate TeaVM C code + compile native for Linux"
    mainClass.set(mainClassName)
    args = mutableListOf("gen_teavm_c", "linux64_teavm_c")
    classpath = sourceSets["main"].runtimeClasspath
}

tasks.register<JavaExec>("runtime_helper_build_project_mac64_teavm_c") {
    group = "jParser"
    description = "Generate TeaVM C code + compile native for macOS"
    mainClass.set(mainClassName)
    args = mutableListOf("gen_teavm_c", "mac64_teavm_c")
    classpath = sourceSets["main"].runtimeClasspath
}

tasks.register<JavaExec>("runtime_helper_build_project_macArm_teavm_c") {
    group = "jParser"
    description = "Generate TeaVM C code + compile native for macOS ARM"
    mainClass.set(mainClassName)
    args = mutableListOf("gen_teavm_c", "macArm_teavm_c")
    classpath = sourceSets["main"].runtimeClasspath
}

tasks.register<JavaExec>("runtime_helper_build_project_android_teavm_c") {
    group = "jParser"
    description = "Generate TeaVM C code + compile native for Android"
    mainClass.set(mainClassName)
    args = mutableListOf("gen_teavm_c", "android_teavm_c")
    classpath = sourceSets["main"].runtimeClasspath
}

tasks.register<JavaExec>("runtime_helper_build_project_ios_teavm_c") {
    group = "jParser"
    description = "Generate TeaVM C code + compile native for iOS"
    mainClass.set(mainClassName)
