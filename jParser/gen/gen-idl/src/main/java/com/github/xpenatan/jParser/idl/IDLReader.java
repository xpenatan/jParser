...
public class IDLReader {
    ...
    private final static String baseRuntimePath = "RuntimeHelper.idl";

    public IDLReader() {
        if(JParser.CREATE_RUNTIME_HELPER) {
            fileArray.add(getRuntimeHelperFile());
        }
    }

    public static IDLFile getRuntimeHelperFile() {
        InputStream resourceAsStream = NativeArray.class.getClassLoader().getResourceAsStream(baseRuntimePath);
        InputStreamReader streamReader = new InputStreamReader(resourceAsStream);
        IDLFile baseIDLFile = IDLReader.parseFile(streamReader, baseRuntimePath);
        return baseIDLFile;
    }
    ...
    public static IDLReader readIDL(IDLFile ...idlFiles) {
        IDLReader reader = new IDLReader();
        for(int i = 0; i < idlFiles.length; i++) {
            IDLFile idlFile = idlFiles[i];
            reader.fileArray.add(idlFile);
        }
    ...
    public static void addIDLRef(IDLReader reader, IDLFile idlFile) {
        idlFile.skip = true;
        reader.fileArray.add(idlFile);
    }
