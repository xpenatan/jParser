...
TeaVM 0.15.0 also emits a Unix fiber runtime that uses POSIX realtime timers and signals, which Apple platforms do not provide. Until the first TeaVM release containing [the upstream Apple runtime fix](https://github.com/konsoletyper/teavm/commit/9c99e0e67d99cbc3a3736bc34a1cc9222ed82059), `runtime-ios-c` packages its `definitions.h` and `fiber.c` backport under `external_cpp/jparser/runtime/teavm/ios`. The same directory contains a `file.c` overlay because iOS simulator app users need not have a passwd-database entry: it resolves the sandbox home from `HOME` before safely falling back to `getpwuid`. An iOS application build overlays all three files onto the generated TeaVM C source directory after generation and before native compilation. The TestLib and SharedLib custom emulator preparation tasks demonstrate this source-preparation step; it does not require another CMake hook or change jParser's launcher boundary.
