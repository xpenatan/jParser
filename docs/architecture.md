...
The Gradle plugin is a standalone nested build rather than a root subproject or an included root build. Root plugin fixtures consume `com.github.xpenatan.jparser` through the shared version-catalog alias. The snapshot repository in root `pluginManagement` uses Maven POM and artifact metadata until published Gradle Module Metadata carries the same generator dependency coordinates as the POM. Do not add `includeBuild("jParser/tools/gradle-plugin")` to the root settings: the plugin build maps generator modules and their transitive jParser modules from the repository for standalone plugin development, and including that build in the root gives IntelliJ two modules for each shared source directory. The duplicate content roots produce long qualified module labels in the Project tree.

The standalone plugin build imports the root `gradle/libs.versions.toml` catalog and uses direct project dependencies on `gen-build`, `gen-idl`, and `gen-build-tool`, with build outputs isolated below the plugin build directory. Its mirrored projects provide only the plugin classpath; their tests remain owned and executed by the root build. The plugin POM writes those dependencies with their normal jParser Maven coordinates and the selected lifecycle version for external consumers. Keep artifact naming in `build.gradle.kts`. The plugin build does not own a separate `buildSrc`; dependency, plugin, publication, and Java target versions come from the shared catalog. Projects intentionally do not set Gradle's `group` or `version`; Easy Publishing exclusively applies the catalog group and selected release or snapshot version to publication projects.
